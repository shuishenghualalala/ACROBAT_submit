...
from network.correlation import Correlation
...
class MaskFlownet(nn.Module):
    def __init__(self, device='cpu',**kwargs):
        ...
        self.MaskFlownet_S = MaskFlownet_S(device = self.device)
    ...
    def forward(self, im1, im2):
        ...
        visuals = []
        visuals.append(flow2[:,:1]) # pred为预测的光流场
        shape = im1.shape
        up_flow = Upsample(preds[-1], 4) # 上采样得到最终的光流场
        up_flow = F.interpolate(up_flow, size=[shape[2], shape[3]], mode='bilinear') * \
                  torch.tensor([shape[d] / up_flow.shape[d] for d in (2, 3)], device=im1.device).view(1, 2, 1, 1)
        final_flow = up_flow.flip(1) # The original MxNet implementation of MaskFlownet predict the flipped flow
        # 位移场 [b,2,h,w]
        warped = warp_imgs(im1,final_flow)
        return warped,final_flow
